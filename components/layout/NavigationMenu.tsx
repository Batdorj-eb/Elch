...
const NavigationMenu: React.FC<NavigationMenuProps> = ({ categories }) => {
  ...
  // ---------- CUSTOM ORDER ----------
  // Энд та өөрийн хүссэн slug дарааллыг оруулна уу.
  // Жишээ:
  ...
  // orderedCategories: customOrder-д байгаа категориуд эхэнд, дараа нь бусад нь
  const orderedCategories = useMemo(() => {
    if (!Array.isArray(categories)) return [];

    // ensure display_order numeric
    const normalized = categories.map((c) => ({
      ...c,
      display_order: typeof c.display_order === 'number' ? c.display_order : 0,
    }));

    // first: those in customOrder (keeping the order specified)
    ...
    const remaining = normalized
      ...
      // If backend provides display_order, prefer that; otherwise sort by name
      .sort((a, b) => {
  ...
  // menuCategories shown on desktop (exclude 'video' if you want)
  const menuCategories = useMemo(
    () => orderedCategories.filter((c) => c.slug !== 'video'),
    [orderedCategories]
  );

  // visibleCategories for mobile links (exclude video)
